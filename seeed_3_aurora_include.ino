...
void ledSky() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    int yPos = i;
    byte noises = inoise8(yPos * zoom, xPos * zoom, aniSpeed);
    leds[i] = ColorFromPalette(CloudColors_p, noises, 255);  //  (color palette  ,rainbow color index, brightness)
  }
}
...
//       leds[j] = leds[j - 1];
//     }
//     leds[0] = ColorFromPalette(auroraa, color + 0.3 * i, 55 + (200 / wavesize * i));
